...
public class Node5 extends BaseNode {

	private static int myServerPort = 5004;
	private	static Node5 clientNodes[] = new Node5[5];

	public Node5(String clientAddress, int clientPort, String nodeName) {
		super(clientAddress, clientPort, nodeName);
	}

	public Node5(int serverPort, String nodeName) {
		super(serverPort, nodeName);
	}

	public static Node5 getClientNode(int clientPort) {
	...
	public static void main(String datas[]) throws Exception {
		...
		Node5 serverNode = new Node5(myServerPort, "Node5");
		serverNode.baseNodeServerInterface = new BaseNodeServerInterface(){
			public void onReceiveCriticalSectionRequest(boolean isConditionSatisfied, int replyBackPort) {
				...
				if(isConditionSatisfied == true) {
					...
					Node5 clientNode = Node5.getClientNode(replyBackPort);
		...
		BaseNode.registerNodeName("Node5", myServerPort);
		...
		for(int i = 0; i < BaseNode.MAX_NUMBER_OF_NODES_ALLOWED; i++) {
			...
			if(clientPort != myServerPort) {
				clientNodes[i] = new Node5("127.0.0.1", clientPort, "Node5");
